...
void flyToLandingPosition(OS_Socket_Handle_t socket, char *buf, uint16_t lidar, float* landingPosition){
    ...
    memcpy(currPosition, buf, sizeof(currPosition));
    ...
    while(distanceToGround > 6){
        sendMoveByRollPitchYawZAsyncCommand(socket, buf, 0, flightPitch, yawAngle, landingPosition[2]-3, 0.05);
        getDistance(socket, buf);
        memcpy(&distanceToGround, buf, sizeof(distanceToGround));
        Debug_LOG_INFO("flyToLandingPositon - distanceToGround: %f", distanceToGround);
    }
...
/* 
        ...
        Args:
            char* buf1              : Pointer to buffer2 containing pointcloud1
            int buf1_len            : Length of buffer1 in bytes
            char* buf2              : Pointer to buffer2 containing pointcloud2
...
static float * detectExactLandingPoint(char* buf1, int buf1_len, char* buf2, int buf2_len, uint16_t lidar1, uint16_t lidar2){
    ...
    if(ignore_lidar == 2 || ignore_lidar == 0 ){
        //middle_point1 = getClosestObjectMiddlePoint() TODO
    }

    if(ignore_lidar == 1 || ignore_lidar == 0){
        //middle_point2 = getClosestObjectMiddlePoint() TODO
    }
